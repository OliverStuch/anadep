...
object DependencyGraphJGraphTAdaptor{ def apply[V] = new DependencyGraphJGraphTAdaptor[V] (
...
class DependencyGraphJGraphTAdaptor [V](val dependencyGraph:DirectedGraph[V, Dependency[V]]) extends DependencyGraph[V]{
  ...
  override def findVertex (v:V) = { // TODO slow and ugly
    val vertices = scala.collection.jcl.Conversions.convertSet(dependencyGraph.vertexSet)
  ...
  override  def foreachEdge (doSomething : Dependency[V] => Unit)  {
    val edges = scala.collection.jcl.Conversions.convertSet(dependencyGraph.edgeSet)
